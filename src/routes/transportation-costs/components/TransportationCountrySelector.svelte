...
<script lang="ts">
  ...
  import { getAllRegions, getCountriesByRegion, getCitiesByCountry } from '$lib/utils/transportation';
  
  // ✅ FIX: Use $props() with $bindable() for two-way binding
  let {
  ...
  const countries = $derived(getCountriesByRegion(selectedRegion));
