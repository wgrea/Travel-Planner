...
<div class="comparison-bar fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-lg border-t border-gray-200 p-4 shadow-lg">
  <div class="max-w-4xl mx-auto flex items-center justify-between">
    <div class="flex gap-4">
      {#each countriesToCompare as countryId}
        {#if minimalData.countries[countryId]}
          <div class="flex items-center gap-2">
            <span class="font-semibold">{minimalData.countries[countryId].name}</span>
            <button on:click={() => removeFromComparison(countryId)} class="text-red-500 hover:text-red-700">
              ❌
            </button>
          </div>
        {/if}
