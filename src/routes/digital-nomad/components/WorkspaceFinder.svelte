...
<script lang="ts">
  ...
  // Filter workspaces - FIXED: Create functions that return arrays
    ...
    {:else}
      ...
      <div class="text-center py-8 text-gray-500">
        ...
        <p class="text-sm text-gray-500">
          {#if selectedCity}
            No {workPreference} options in {selectedCity}
          {:else}
            No {workPreference} options available
          {/if}
        </p>
      </div>
    {/if}
